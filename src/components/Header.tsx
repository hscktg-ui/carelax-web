...
export default function Header() {
  ...
  return <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${solid ? "bg-purple-deep/95 text-white backdrop-blur-md shadow-[0_1px_0_rgba(0,174,189,0.25)]" : "text-white"}`}>
    <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5 md:h-20 md:px-8">
      ...
      <nav className="hidden items-center gap-6 md:flex" aria-label="주요 탐색">
        ...
        <a href={SITE.smartStoreUrl} target="_blank" rel="noopener noreferrer" className="rounded-sm bg-cyan px-4 py-2 text-sm font-bold text-purple-deep transition hover:bg-cyan-soft">스마트스토어</a>
      </nav>
    ...
    {open && <div className="border-t border-cyan/20 bg-purple-deep px-5 py-6 md:hidden"><nav className="flex flex-col gap-4" aria-label="모바일 탐색">
      ...
      <a href={SITE.smartStoreUrl} target="_blank" rel="noopener noreferrer" className="mt-2 w-fit rounded-sm bg-cyan px-4 py-2 text-sm font-bold text-purple-deep">스마트스토어</a>
    </nav></div>}
  </header>;
}